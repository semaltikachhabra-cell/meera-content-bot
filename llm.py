...
FALLBACK_MODELS = ["gemini-flash-lite-latest", "gemini-3.8-flash", "gemini-flash-latest"]
...
def gemini(prompt, system=None, as_json=False, temperature=0.4, audio=None):
    ...
    # If a model is overloaded (503), rate-limited (429) or retired (404), fall back to the next one.
    ...
    for i, model in enumerate(models):
        try:
            ...
            if i == len(models) - 1 or not any(f"-> {c}" in str(e) for c in ("503", "429", "404", "500")):
                raise
    try:
